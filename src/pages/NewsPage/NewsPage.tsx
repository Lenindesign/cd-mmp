...
const DealCardImage = ({ src, alt, badge }: DealCardImageProps) => {
  const [imageFailed, setImageFailed] = useState(!src);

  return (
    <div className="news-page__deal-card-image">
      {imageFailed ? (
        <span className="news-page__deal-card-image-missing">Image unavailable</span>
      ) : (
        <img src={src} alt={alt} onError={() => setImageFailed(true)} />
      )}
      <span className="news-page__deal-card-badge">{badge}</span>
    </div>
  );
};

const NewsBreakerAd = () => (
  <div className="news-page__breaker-ad" role="complementary" aria-label="Advertisement">
...
export const NewsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateParallax = () => {
      ...
      hero.style.setProperty('--news-hero-parallax', `${(clamped - 0.5) * 72}px`);
    };
    ...
    return () => {
      ...
      hero.style.removeProperty('--news-hero-parallax');
    };
  }, []);
  ...
  return (
    <div className="news-page">
      {/* Hero */}
      <section className="news-page__hero" ref={heroRef}>
      ...
      <div className="news-page__rows">
        <div className="container">
          ...
          <HomeCarouselSection
            title="Best Deals"
            meta={`${bestDeals.length} featured`}
            href="/deals"
          >
            {bestDeals.map(deal => (
